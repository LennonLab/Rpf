...
table.phylo4d(x, treetype = "phylo", symbol = "colors", show.node = TRUE, 
              ...
              col=mypalette(25), pch = 15, cex.symbol = 2.5,
              ratio.tree = 0.81, cex.legend = 1, center = TRUE)
```
...
lambda.a.model.0 <- fitContinuous(ml.lambda.0, traits.norm.na, model = "lambda")
...
# P-value are A = 0.052, umax = 0.392, L = 0.669. 
...
tree <- plotTree.barplot(ml.rooted,traits, args.barplot = list(beside=TRUE, xlim = c(-1,1), 
    legend.text=TRUE,space=c(0,1.5),args.legend=list(x=-0.45,y=17),
    xlab="% Change in growth parameter with Rpf"))
...
# Two factor ANOVA # 
L1 <- aov(L ~ Treatment*Genus, data=gc)
summary(L1)
...
# Final biomass yeild # 
# Two factor ANOVA # 
a1 <- aov(A~Treatment*Genus, data=gc)
summary(a1)
#TukeyHSD(a1)
...
plot.new()
#par(mfrow=c(1,3))
plot(traits.norm.db$A~pd0714, ordered, xlab = "Phylogenetic distance to Micrcoccus", ylab = "Normalized proportonal change of biomass yeild", main = "Rpf effect on A"  )
...
plot.new()
# Maximum growth rate # 
plot(traits.norm.db$umax~pd0714, ordered, xlab = "Phylogenetic distance to Micrcoccus", ylab = "Normalized proportonal change of umax", main = "Rpf effect on umax" )
...
# Lag time # 
plot.new()
plot(traits.norm.db$L~pd0714, ordered, xlab = "Phylogenetic distance to Micrcoccus", ylab = "Normalized proportonal change of L", main = "Rpf effect on L")
...
```{r}
# different way of plotting line graph
cfu.line <- ggplot(data=ordered, aes(x=pd0714, y=A)) + 
  geom_line()


# Line graph CFU # 
cfu.line <- ggplot(data=ordered, aes(x=pd0714, y=L)) + 
  geom_line()

cfu.plot <- cfu.line +    # Figure editting #
  labs(x=expression(paste("Phylogenetic distance to Micrococcus sp.")), 
       y="Proportional change in lag time") +
  theme_classic() +
  theme(axis.text.y=element_text(colour="black", size=20, margin =margin(0,15,0,0)),
        ...
        axis.title.y = element_text(colour="black", size=15,  margin = margin(0,20,0,10)),
