...
#Treatment         1 0.00000 0.000001   0.015 0.9043    
#Strain           11 0.03751 0.003410  82.567 <2e-16 ***
#Treatment:Strain 11 0.00079 0.000072   1.747 0.0802 .  
#Residuals        72 0.00297 0.000041               
...
# Treatment      4.2900e-08 4.2900e-08     1    82  0.0009 0.9757
# Gram           3.8459e-06 3.8459e-06     1    10  0.0840 0.7779
# Treatment:Gram 1.1201e-05 1.1201e-05     1    82  0.2446 0.6223

# No Gram + vs. Gram - effects on yield
...
# Highly significant effect of Rpf (F[1,82]= 45.6297, P = 1.908e-09)
# Marginally significant main effect of Gram (F[1,10]= 8.73, P =  0.08114)
...
# Highly significant effect of Rpf (F[1,82]= 22.407, P = 9.102e-06)
# Significant main effect of Gram (F[1,10]= 8.856, P = 0.013905)
...
#    -0.08406169 0.3295956  0.9989581 0.2814522 0.8458633 0.3223238
# summary:
# weak overall effect for yield
# strong overall positive effect on umax
# strong overall positive effect on lag
...
# neg          0.669  0.339       1.67    0.0934      1.65    0.0780
# pos         -0.622  0.414       0.519   0.391       0.268   0.436 

# umax: "medium" positive for Gram + (0.519); "large" positive for Gram - (1.67)
# lag: "strong" positive for Gram - (1.65); "small-medium" positive for Gram + (0.268)
# yield: "medium" positive for Gram - (1.67); "medium" negative for  Gram + (0.691)
# Summary: Rpf stimulated Gram - bacteria in terms of higher yield and umax, but longer lag
# Summary: Rpf reduced lag time for Gram + bacteria, but also reduced yield
```
...
print(result_K_A)
#Phylogenetic signal K :  0.28
# P-value (based on 1000 randomizations) : 0.403 
# K = 0.28 consistent with overdispersion of Rpf effect
# But P = 0.403, so not significant

# Run test for Blomberg's K for umax
result_K_umax <- phylosig(tree, cohen.d3[, 2], method = "K", test = TRUE) 
print(result_K_umax)
#Phylogenetic signal K : 0.36 
#P-value (based on 1000 randomizations) : 0.342 
# K = 0.36 1 consistent with overdispersion of Rpf effect
# P = 0.342 so not significant

# Run test for Blomberg's K for lag
result_K_lag <- phylosig(tree, cohen.d3[, 3], method = "K", test = TRUE) 
print(result_K_lag)
# Phylogenetic signal K : 0.986728 consistent with Brownian motion
# P-value (based on 1000 randomizations) : 0.016  
# But P = 0.016, significant
...
# lambda =  0.999927, P value = 0.0130492 ==> consistent with Brownian motion
...
#text(-2.2, 3, labels = expression(r^2 == 0.50), cex = 1.25)
text(-2.1, 2, labels = expression(italic("P")~" = 0.014"), cex = 1.25)

##########################
# Panel 2: MPamin for lag 
##########################

# calculate regressions: umax effect vs. Mpamin
fit.lag.mpamin <- lm(lag.effect ~ Mpamin, data = g.traits)
pred.frame.lag.mpamin <- data.frame(Mpamin = seq(-2.4, -0.01, by = 0.05))
# F(1,9) = 2.823, p-value: 0.01996, r2, = 0.47

# create minimum plot 
plot(Mpamin ~ lag.effect, g.traits, axes = F, type = "n",
     xlab = "", ylab = "",
     xlim = c(-2.6, 0), ylim = c(-3.5, 3.5), las = 1)
box(lwd = 2)

mtext(expression(bold(Lag)), side = 2, line = -10, at = 6, cex = 1.75, las = 2)

# Add hull confidence hulls and regression line
add.hull(fit.lag.mpamin, pred.frame.lag.mpamin)
matlines(pred.frame.lag.mpamin, predict(fit.lag.mpamin, 
      interval = "c", newdata = pred.frame.lag.mpamin), lty = c(3, 3, 3), 
      lwd = c(2, 0, 0), col = "red")

# Add points
points(lag.effect ~ Mpamin, g.traits,
       pch = 21, col = "gray42", bg = "gray42", lwd = 2, cex = 1.5)
...
# F(1,9) =  -1.983, p-value: 0.07873, r2, = 0.2266 
...
# Add stats and panel labels
text(0.05, -2, labels = expression(r^2 == 0.23), cex = 1.25)
...
# calculate regressions
fit.lag.b <- lm(lag.effect ~ b, data = g.traits)
pred.frame.lag.b <- data.frame(b = seq(-0.16, 1.28, by = 0.1))
# F(1,9) = -3.107, p-value: 0.013, r2, = 0.52 

# create minimum plot 
plot(b ~ lag.effect, g.traits, axes = F, type = "n",
     xlab = "", ylab = "",
     xlim = c(-0.2, 1.25), ylim = c(-3.5, 3.5), las = 1)
box(lwd = 2)

# Add hull confidence hulls and regression line
add.hull(fit.lag.b, pred.frame.lag.b)
matlines(pred.frame.lag.b, predict(fit.lag.b, 
      interval = "c", newdata = pred.frame.lag.b), lty = c(3, 3, 3), 
      lwd = c(2, 0, 0), col = "red")

# Add points
points(lag.effect ~ b, g.traits,
       pch = 21, col = "gray42", bg = "gray42", lwd = 2, cex = 1.5)
...
# Add stats and panel labels
text(0.05, -2, labels = expression(r^2 == 0.52), cex = 1.25)
...
# F(1,9) = 2.63, p-value: 0.02767, r2, = 0.43
...
text(-0.02, -2, labels = expression(r^2 == 0.54), cex = 1.25)
text(-0.04, -2.85, labels = expression(italic("P")~" = 0.01"), cex = 1.25)
...
# calculate regressions
fit.lag.Wopt <- lm(lag.effect ~ Wopt, data = g.traits)
pred.frame.lag.Wopt <- data.frame(Wopt = seq(-1.25, 0.2, by = 0.1))
# F(1,9) = 3.007, p-value: 0.01478, r2, = 0.50

# create minimum plot 
plot(Wopt ~ lag.effect, g.traits, axes = F, type = "n",
     xlab = "", ylab = "",
     xlim = c(0.2, -1.25), ylim = c(-3.5, 3.5), las = 1)
box(lwd = 2)

# Add hull confidence hulls and regression line
add.hull(fit.lag.Wopt, pred.frame.lag.Wopt)
matlines(pred.frame.lag.Wopt, predict(fit.lag.Wopt, 
      interval = "c", newdata = pred.frame.lag.Wopt), lty = c(3, 3, 3), 
      lwd = c(2, 0, 0), col = "red")

# Add points
points(lag.effect ~ Wopt, g.traits,
       pch = 21, col = "gray42", bg = "gray42", lwd = 2, cex = 1.5)
...
text(-0.02, -2, labels = expression(r^2 == 0.50), cex = 1.25)
