...
# Add basline control biomass
E54A.base = od.table$mean[ which(od.table$Treatment == "E54ALMM") ]
abline(h = E54A.base ,color= 'Black',  lwd =2, lty = 2)
...
# Add basline control biomass
E54K.base = od.table$mean[ which(od.table$Treatment == "E54KLMM") ]
abline(h = E54K.base ,color= 'Black', lwd =2,  lty = 2)
...
# Add basline control biomass
E54Q.base = od.table$mean[ which(od.table$Treatment == "E54QLMM") ]
abline(h = E54Q.base ,color= 'Black', lwd =2,  lty = 2)
...
# Add basline control biomass
trnc.base = od.table$mean[ which(od.table$Treatment == "TrncLMM") ]
abline(h = trnc.base , color= 'Black', lwd =2, lty = 2)
...
# Define the Outgroup
#outgroup <- match("Prochlorococcus", nj.tree$tip.label)

# Create a Rooted Tree {ape}
#nj.rooted <- root(nj.tree, outgroup, resolve.root = TRUE)

# Keep Rooted but Drop Outgroup Branch
#nj.rooted <- drop.tip(nj.rooted, "Prochlorococcus")
...
# Define the outgroup
outgroup <- match("Prochlorococcus", ml.tree$tip.label)

# Create a rooted tree {ape}
ml.rooted <- root(ml.tree, outgroup, resolve.root = TRUE)

# Keep rooted but drop outgroup branch
ml.rooted <- drop.tip(ml.rooted, "Prochlorococcus")
